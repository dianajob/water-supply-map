...
export const useConvertData = system => {
  const subsystems = [];
  const coordinates = [];
  ...
  const addCoordinate = (id, type, subtype, latlong, subsystem, output_devices, shift_index) => {
    coordinates.push({
      id,
      type,
      subtype,
  ...
  for (let sub of subsystems) {
    for (let tank of sub.tanks) {
      tank.sensors.forEach(s => addSensor(s.id));
      tank.devices.forEach(d => d.sensors.forEach(s => addSensor(s.id)));
      addCoordinate(
        tank.id,
        tank.class_type,
        null,
    ...
    for (let pump of sub.pumpSt) {
      pump.sensors.forEach(s => addSensor(s.id));
      pump.devices.forEach(d => d.sensors.forEach(s => addSensor(s.id)));
      addCoordinate(
        pump.id,
        pump.class_type,
        pump.subtype,
    ...
    for (let cons of sub.consP) {
      cons.sensors.forEach(s => addSensor(s.id));
      addCoordinate(
        cons.id,
        cons.class_type,
        null,
  ...
  return { subsystems, coordinates, sensors, pipes, subpipes };
};
